...
public:
	...
	/** 返回当前保存的游戏，所以可以被用来初始化状态。直到 WriteSaveGame 被调用，改变才会写入。 Returns the current save game, so it can be used to initialize state. Changes are not written until WriteSaveGame is called */
	...
	/** 把当前保存的游戏写入硬盘，在后台线程中进行。 Writes the current save game object to disk. The save to disk happens in a background thread*/
	UFUNCTION(BlueprintCallable, Category = Save)
	bool WriteSaveGame();
